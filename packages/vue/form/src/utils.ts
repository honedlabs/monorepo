...
export function getComponent(formComponent: Component, errors: Record<string, string> = {}): VNode {
    const { component, attributes = {}, ...props } = formComponent

    const { schema, ...rest } = props as Grouping

    const additional = 'name' in rest ? { error: errors[(rest as Field).name] } : {}
...
export function getComponents(schema?: Component[], errors?: Record<string, string>): VNode[] {
    if (schema === undefined)
        return []
