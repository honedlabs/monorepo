...
import type { DefineComponent, Plugin, PropType, Raw } from "vue";
import { useForm, useRemember } from "@inertiajs/vue3";
import type { VuePageHandlerArgs } from "@inertiajs/vue3";
...
const page = ref<Page<any> | null>(null);
...
export function usePage<SharedProps extends PageProps>(): Page<SharedProps> {
	return reactive({
		...
		layout: computed(() => page.value?.layout),
