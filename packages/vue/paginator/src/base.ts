...
>(
    ...
    const source = computed<SimplePaginator<Data> | SimplePaginatorResource<Data>>(() => {
        if (isKey) {
            return (dataOrProps as Props)[keyOrOptions] as unknown as SimplePaginatorResource<Data>;
        }
    ...
    const defaultOptions = {
        ...
        ...(isResource.value ? {
            only: [...((isKey ? (keyOrOptions as any).only : []) as string[]), ...(isKey ? [keyOrOptions] : [])],
        } : {}),
    }
