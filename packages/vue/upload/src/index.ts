...
export const useUpload = (url: string, defaultOptions: UploadOptions = {}) => {
    ...
    */
     ...
     */
    async function upload(file: File, options: Omit<UploadOptions, 'waited' | 'files'> = {}) {
        const onEvent = (event: keyof Omit<UploadOptions, 'waited' | 'meta' | 'files'>, arg?: any) => {
        ...
        axios.post(url, {
            ...
            axios.post(data.attributes.action, formData, {
                ...
                ({ data }) => onEvent('onSuccess', data)
            ).catch(
                (error) => onEvent('onError', error)
            )
        }).catch((error) => {
